...
def objective(beta, X, y, reg_param):
    log_likelihood = np.sum(np.log1p(np.exp(np.dot(X, beta))) - y * np.dot(X, beta)) 
    l2_term = reg_param * np.dot(beta, beta)
    return log_likelihood + l2_term

def gradient_vector(beta, X, y, reg_param):
    gradiant =  np.dot(X.T , (sigmoid(np.dot(X,beta)) - y)) + 2 * reg_param * beta #  X.t*(sigmoid(X*beta)-y)+2*λ*beta
    return gradiant

def wolfe_line_search(beta, direction, X, y, r, grad_current, objective_func, gradient_func, c1=1e-4, c2=0.9, step_init=1.0, max_ls_iters=50):
    step = step_init 
    ...
    for i in range(max_ls_iters):
        ...
        if f_new > f_k + c1 * step * grad_dot_direction:
            step = step * 0.5 
            continue 
...
def BFGS(feature_matrix, labels, reg_param_r, initial_beta_guess, max_iterations=100, convergence_tolerance=1e-5):
    ...
    obj_values = [objective(beta_k, feature_matrix, labels, reg_param_r)]
    grad_norms = [np.linalg.norm(gradient_k)]
    
    print("初始目标函数值 {:.6f}".format(obj_values[0]))
    print("初始梯度范数: {:.6f}".format(grad_norms[0]))
    ...
    while iteration_count < max_iterations:
        ...
        obj_values.append(objective(beta_k, feature_matrix, labels, reg_param_r))
        grad_norms.append(np.linalg.norm(gradient_k))
    ...
    print(f"最终目标函数值: {obj_values[-1]:.6f}")
    print(f"最终梯度范数: {grad_norms[-1]:.6f}")
    print(f"总耗时: {total_time:.2f} 秒")
    

    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    plt.plot(obj_values, label='Objective Value')
...
if __name__ == '__main__':
    ...
    for p1 in p:
        ...
        if p1 > 100:
            ...
            tolerance = 1e-5
            iteration = 1000
        BFGS(feature_matrix=x, labels=y, reg_param_r=r, initial_beta_guess=beta, max_iterations=iteration, convergence_tolerance=tolerance)
